...
func Map(lc Lifecycle, agent, sourceAgent, eventID string, payload map[string]any) model.Event {
	return mapEvent(lc, agent, sourceAgent, eventID, payload)
}

func mapEvent(lc Lifecycle, agent, sourceAgent, eventID string, payload map[string]any) model.Event {
	...
	case LifecyclePromptSubmit:
		...
		ev.SetContent(promptWithAttachments(prompt, r.attachmentPaths()), true)
