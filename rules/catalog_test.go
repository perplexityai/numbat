...
func TestTamperRules(t *testing.T) {
	...
	runCases(t, eng, []ruleCase{
		...
		{"project mcp.json write", write("/repo/.mcp.json"), "tamper.agent_config_write"},
		// MCP server definitions are a config surface in their own right: an entry
		// names a command numbat's host runs on every later session, and the tool
		// descriptions it serves reach the model. These are the documented stores
		// for agents whose hook file this rule already covers.
		{"Claude user-scope MCP write", write("/Users/dev/.claude.json"), "tamper.agent_config_write"},
		{"Claude managed MCP write", write("/Library/Application Support/ClaudeCode/managed-mcp.json"), "tamper.agent_config_write"},
		{"Claude managed MCP Linux write", write("/etc/claude-code/managed-mcp.json"), "tamper.agent_config_write"},
		{"Claude managed MCP Windows write", write("C:/Program Files/ClaudeCode/managed-mcp.json"), "tamper.agent_config_write"},
		{"Claude managed MCP delete restores user control", model.Event{EventType: model.EventFileDelete, FilePath: "/etc/claude-code/managed-mcp.json"}, "tamper.agent_config_write"},
		{"Cursor user MCP write", write("/Users/dev/.cursor/mcp.json"), "tamper.agent_config_write"},
		{"Cursor project MCP write", write("/repo/.cursor/mcp.json"), "tamper.agent_config_write"},
		{"Codex user config write", write("/Users/dev/.codex/config.toml"), "tamper.agent_config_write"},
		{"Windsurf MCP config write", write("/Users/dev/.codeium/windsurf/mcp_config.json"), "tamper.agent_config_write"},
		{"VS Code workspace MCP write", write("/repo/.vscode/mcp.json"), "tamper.agent_config_write"},
		...
		{"redirect over Cursor MCP config", cmd(`printf '{}' > ~/.cursor/mcp.json`), "tamper.agent_config_write"},
		{"append MCP server to Codex config", cmd(`printf '[mcp_servers.x]\n' >> ~/.codex/config.toml`), "tamper.agent_config_write"},
		{"tee Claude user-scope MCP config", cmd(`printf '{}' | tee ~/.claude.json`), "tamper.agent_config_write"},
		{"install Claude managed MCP", cmd("sudo install -m 0644 /tmp/mcp.json '/Library/Application Support/ClaudeCode/managed-mcp.json'"), "tamper.agent_config_write"},
		{"remove Claude managed MCP", cmd("sudo rm -f /etc/claude-code/managed-mcp.json"), "tamper.agent_config_write"},
		{"copy over Windsurf MCP config", cmd("cp /tmp/mcp_config.json ~/.codeium/windsurf/mcp_config.json"), "tamper.agent_config_write"},
		{"PowerShell set VS Code workspace MCP", cmd(`Set-Content .vscode\mcp.json '{}'`), "tamper.agent_config_write"},
		...
		{"reading MCP config is not tampering", model.Event{EventType: model.EventFileRead, FilePath: "/Users/dev/.cursor/mcp.json"}, ""},
		{"MCP config backup untouched", write("/Users/dev/.cursor/mcp.json.bak"), ""},
		{"unrelated nested mcp.json untouched", write("/repo/fixtures/mcp.json"), ""},
		{"Claude config name lookalike untouched", write("/repo/claude.json"), ""},
		{"relocated Codex home is not guessed", write("/opt/codex/config.toml"), ""},
		{"unrelated vscode setting untouched", write("/repo/.vscode/settings.json"), ""},
		{"relocated managed MCP is not guessed", write("/opt/ClaudeCode/managed-mcp.json"), ""},
