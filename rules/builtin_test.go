...
func TestPrivilegeKernelMemoryAccess(t *testing.T) {
	eng := builtinEngine(t)
	runCases(t, eng, []ruleCase{
		{"read physical memory", cmd("sudo cat /dev/mem"), "privilege.kernel_memory_access"},
		{"read physical memory through an absolute program path", cmd("sudo /bin/cat /dev/mem"), "privilege.kernel_memory_access"},
		{"read physical memory with a quoted path", cmd(`sudo cat "/dev/mem"`), "privilege.kernel_memory_access"},
		{"bounded read of physical memory", cmd("sudo head -c 4096 /dev/mem"), "privilege.kernel_memory_access"},
		{"page through physical memory", cmd("sudo less /dev/mem"), "privilege.kernel_memory_access"},
		{"hex dump of kernel memory", cmd("sudo xxd /dev/kmem"), "privilege.kernel_memory_access"},
		{"canonical hex dump of physical memory", cmd("sudo hexdump -C /dev/mem"), "privilege.kernel_memory_access"},
		{"octal dump of the kernel core image", cmd("sudo od -A x -t x1z -v /proc/kcore"), "privilege.kernel_memory_access"},
		{"strings over the kernel core image", cmd("sudo strings /proc/kcore"), "privilege.kernel_memory_access"},
		{"encode the kernel core image", cmd("sudo base64 /proc/kcore"), "privilege.kernel_memory_access"},
		{"copy the kernel core image out", cmd("sudo cp /proc/kcore /tmp/kcore.dump"), "privilege.kernel_memory_access"},
		{"copy a patch onto physical memory", cmd("sudo cp /tmp/patch /dev/mem"), "privilege.kernel_memory_access"},
		{"dump physical memory with dd", cmd("sudo dd if=/dev/mem bs=1 skip=1048576 count=64"), "privilege.kernel_memory_access"},
		{"patch physical memory with dd", cmd("sudo dd of=/dev/mem bs=1 seek=4096 count=1"), "privilege.kernel_memory_access"},
		{"dd with flags before the input operand", cmd("sudo dd iflag=fullblock if=/dev/mem of=/tmp/dump"), "privilege.kernel_memory_access"},
		{"pipe a patch into physical memory", cmd("cat /tmp/patch | sudo tee /dev/mem"), "privilege.kernel_memory_access"},
		{"read redirect from physical memory", cmd("sudo cat < /dev/mem"), "privilege.kernel_memory_access"},
		{"write redirect to physical memory", cmd("printf x > /dev/mem"), "privilege.kernel_memory_access"},
		{"append redirect to kernel memory", cmd("printf x >> /dev/kmem"), "privilege.kernel_memory_access"},
		{"read-write descriptor on physical memory", cmd("exec 3<>/dev/mem"), "privilege.kernel_memory_access"},
		{"combined output redirect to physical memory", cmd("sudo cat /tmp/patch &> /dev/mem"), "privilege.kernel_memory_access"},
		{"read through a shell wrapper", cmd(`sudo sh -c 'strings /proc/kcore'`), "privilege.kernel_memory_access"},
		{"own-root prefix names the same file", cmd("sudo cat /proc/self/root/dev/mem"), "privilege.kernel_memory_access"},
		{"host PID root prefix names the same file", cmd("sudo cat /proc/1/root/proc/kcore"), "privilege.kernel_memory_access"},
		{"doubled slash names the same file", cmd("sudo cat //dev/mem"), "privilege.kernel_memory_access"},
		{"busybox applet read", cmd("sudo busybox cat /dev/mem"), "privilege.kernel_memory_access"},
		{"busybox applet dump", cmd("sudo busybox dd if=/proc/kcore of=/tmp/kcore.dump"), "privilege.kernel_memory_access"},
		{"device listing discloses no memory", cmd("ls -la /dev/mem /dev/kmem"), ""},
		{"ordinary device directory listing stays quiet", cmd("ls /dev"), ""},
		{"metadata inspection stays quiet", cmd("stat /dev/mem"), ""},
		{"file type probe stays quiet", cmd("file /proc/kcore"), ""},
		{"existence test stays quiet", cmd("test -r /dev/mem && echo yes"), ""},
		{"byte count discloses no memory contents", cmd("wc -c /proc/kcore"), ""},
		{"help output is not a read", cmd("xxd --help /dev/mem"), ""},
		{"dd help output is not a read", cmd("dd --help if=/dev/mem"), ""},
		{"here string names the device but reads nothing", cmd("cat <<< /dev/mem"), ""},
		{"literal heredoc body stays quiet", cmd("cat <<'EOF'\ncat /dev/mem\nEOF\n"), ""},
		{"expanding heredoc body stays quiet", cmd("cat <<EOF\ncat /dev/mem\nEOF\n"), ""},
		{"label mention stays quiet", cmd(`echo "=== /dev/mem ==="`), ""},
		{"quoted example stays quiet", cmd(`echo 'sudo cat /dev/mem'`), ""},
		{"memory read in a commit message stays quiet", cmd(`git commit -m "document sudo cat /dev/mem"`), ""},
		{"search pattern is not a path", cmd(`grep -iE "/dev/mem" /tmp/tool-output.txt`), ""},
		{"find name argument is not a path", cmd("find / -name /dev/mem"), ""},
		{"argument mention is not a read", cmd("tr /dev/mem x"), ""},
		{"unexpanded variable stays quiet", cmd(`cat '$MEMPATH'`), ""},
		{"process cmdline is not memory", cmd("cat /proc/1/cmdline"), ""},
		{"memory statistics are not memory", cmd("cat /proc/meminfo"), ""},
		{"kernel command line is not memory", cmd("cat /proc/cmdline"), ""},
		{"kernel symbol table is out of scope", cmd("cat /proc/kallsyms"), ""},
		{"debugfs tracing is out of scope", cmd("cat /sys/kernel/debug/tracing/trace"), ""},
		{"zero device is not memory", cmd("dd if=/dev/zero of=/tmp/testfile bs=1M count=10"), ""},
		{"similar device name stays quiet", cmd("cat /dev/memfoo"), ""},
		{"repository fixture path stays quiet", cmd("cat /repo/dev/mem"), ""},
		{"device path inside a URL stays quiet", cmd("curl https://example.com/dev/mem"), ""},
		{"busybox applet without an operand stays quiet", cmd("busybox cat"), ""},
	})
}
